...
std::string MergeComponent::getModuleInstantiation(std::string tabs){
	setInputPortBus();
	setOutputPortBus();
...
std::string MergeComponent::getVerilogParameters(){
	std::string ret;
	//This method of generating module parameters will work because Start node has
	//only 1 input and 1 output
	ret += "#(.INPUTS(" + std::to_string(in.size) + "), .OUTPUTS(1), ";
...
std::string MergeComponent::getInputOutputConnections(){
	std::string ret;
